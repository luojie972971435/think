...
class AdminRule extends Model
{
	 ...
	 * @return array
	 */
	public static function systemPage($where = [], $page = 0, $field = '*', $order = 'create_time asc,id asc')
	{
	 ...
	 * @return array
	 */
	public static function getList($where = [], $field = '*', $order = 'create_time asc,id asc')
	{
		$list = self::with('pGetInfo')->field($field)->where($where)->order($order)->select();
		if (!empty($list)) {
			if (is_object($list)) $list = $list->toArray();
			foreach ($list as $k => &$val) {
				$list[$k]['p_title'] = empty($val['p_title']) ? '主节点' : $val['p_title'];
			}
			return $list;
		} else {
			return [];
	...
	{
		return $this->hasOne('AdminRule', 'id', 'pid')->bind(['p_title' => 'title']);
	}
}
