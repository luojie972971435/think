...
class Admin extends Model
{
	 ...
	 * @return array
	 */
	public static function systemPage($where = [], $page = 0, $field = 'admin.id,admin.username,admin.status,admin.last_login_time,admin.last_login_ip,admin.login_num,admin.realname,admin.tel,group.title as groupName', $order = 'admin.create_time asc')
	{
		$conf = Config::get("system.");
		$rows = empty($page) ? $conf['LIST_ROWS'] : $page;
		$list = self::field($field)
			->alias('admin')
			->join('admin_group_access access', 'access.uid = admin.id')
			->join('admin_group group', 'group.id = access.group_id')
			->where($where)
			->order($order)
	...
	public static function getList($where = [], $field = '*', $order = 'id desc')
	{
		...
		if (!empty($list)) {
			...
			foreach ($list as $k => &$val) {
				$list[$k]['last_login_time'] = date('Y-m-d H:i:s', $val['last_login_time']);
				$list[$k]['last_login_ip'] = long2ip($val['last_login_ip']);
			}
			return $list;
		} else {
			return [];
	...
	{
		$param['salt'] = create_randomstr(13);
	 ...
	 * @return array
	 */
	public static function getInfos($where = [], $field = 'admin.*,group.title as groupName,access.group_id')
	{
		$detail = self::field($field)
			->alias('admin')
			->join('admin_group_access access', 'access.uid = admin.id')
			->join('admin_group group', 'group.id = access.group_id')
			->where($where)->find();
	 ...
	 * @return array
	 */
	public static function getInfoGroupId($where = [], $field = 'admin.*,access.group_id')
	{
		$detail = self::field($field)
			->alias('admin')
			->join('admin_group_access access', 'access.uid = admin.id')
